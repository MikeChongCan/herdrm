...
final class TailcatMobileTransport: MobileTransport {
    ...
    func openSFTP(timeout _: Duration) async throws -> SSHSFTPClient {
        throw MobileTransportError.tailcatTerminalUnsupported
    }

    func execute(_: String, timeout _: Duration) async throws -> SSHExecResult {
        throw MobileTransportError.tailcatTerminalUnsupported
    }

    func isHealthy() async -> Bool {
        do {
            _ = try await request(method: "ping", params: .object([:]))
            return true
        } catch {
            return false
        }
